        ...
        **kwargs
        ...
        self.stem_block = ModuleList([
            ...
            Sequential(
                ...
                ReLU(inplace=True),
                GroupNorm(num_groups=self.norm_num_groups, num_channels=self.d_model//2, **factory_kwargs)
            ),
            Sequential(
                Conv2d(opts=opts, in_channels=self.d_model//2, out_channels=self.d_model, **stem_conv_kwargs),
                ReLU(inplace=True),
                GroupNorm(num_groups=self.norm_num_groups, num_channels=self.d_model, **factory_kwargs)
            )
        ])
